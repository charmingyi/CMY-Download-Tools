...
import { Download, CheckCircle, AlertCircle, Pause, Play, Terminal, ChevronDown, ChevronUp } from 'lucide-react';
            ...
            <div className="p-4 flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                 ...
                 <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden mb-1">
                    <div className="h-full bg-black transition-all duration-500" style={{ width: \`\${task.progress}%\` }} />
                 </div>
              ...
              <div className="flex items-center gap-2">
                 {(task.status === 'downloading' || task.status === 'pending') && (
                    <button onClick={() => handlePause(task.id)} className="p-2 bg-gray-100 rounded-lg"><Pause className="w-4 h-4"/></button>
                 )}
                 {(task.status === 'paused' || task.status === 'error') && (
                    <button onClick={() => handleResume(task.id)} className="p-2 bg-gray-100 rounded-lg"><Play className="w-4 h-4"/></button>
                 )}
              </div>
            </div>
            {expandedId === task.id && (
